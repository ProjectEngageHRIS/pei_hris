        ...
        <div class=" px-1 mx-auto pt-8">
            ...
            <div>
                ...
                {{-- <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full h-1/2 place-items-center"> --}}
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 16px; width: 100%; height: 50%; justify-items: center; align-items: top;">
                    @foreach ($ActivitiesData as $data)
                        <div class="h-1/2 object-cover">
                            {{-- @php
                                $photo = $this->getActivityPhoto($data->activity_id);
                            @endphp --}}
                            <a href="{{route('ActivitiesView', ['index' => $data->activity_id])}}"><img class="h-full w-full rounded-lg" style="height: 300px;" src="{{ asset('storage/'. $data->poster) }}" alt=""></a>
                        </div>
                    @endforeach
                </div>
