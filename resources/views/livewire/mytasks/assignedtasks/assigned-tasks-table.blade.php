    ...
    <div class="p-4 rounded-lg dark:border-gray-700">
        <nav class="flex mb-4" aria-label="Breadcrumb">
            ...
            <li>
                ...
                <a href="{{route('AssignedTasksTable')}}" class="text-sm font-semibold text-gray-900 ms-1 hover:text-customRed md:ms-2 dark:text-gray-400 dark:hover:text-white">Assigned Tasks</a>
                </div>
            </li>
            </ol>
        </nav>
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td class="items-center py-4 text-center">
                                        <div class="flex items-center space-x-2">
                                            <!-- View Button -->
                                            <a onclick="location.href='{{ route('AssignedTasksView', ['index' => $my_task->uuid]) }}'" class="inline-flex items-center px-4 py-2 text-sm font-medium text-yellow-400 hover:text-yellow-600 ">
                                                View
                                            </a>
                                            <!-- Cancel Button -->
                                            @if ($my_task->status != "Cancelled" && $my_task->status != "Approved" )
                                                <a id="cancel_button_{{ $loop->index }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-red-500 hover:text-red-600" @click="openCancelModal('{{ $loop->index}}')">
                                                    Cancel
                                                </a>
                                            @endif
                                        </div>
